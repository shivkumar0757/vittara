class CreateTransactionTool < ApplicationTool
  ...
  arguments do
    required(:account_id).filled(:integer).description("Account ID to log transaction against")
    required(:amount).filled(:float).description("Amount — positive=expense, negative=income. Or use nature param.")
  ...
  def call(account_id:, amount:, date:, name:, nature: nil, category_id: nil, notes: nil, currency: nil)
    ...
    signed_amount = calculate_signed_amount(amount, nature)
